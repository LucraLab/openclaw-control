...
#   oc_require_agent_id         — validates OC_AGENT_ID env var, returns 1 if bad
...
#   oc_migrate_legacy_file <legacy> <target> — safe one-time migration
#
...
# Require OC_AGENT_ID to be set and valid.
# Call this at the top of production scripts after sourcing libs.
# Returns 1 with error message if invalid.
oc_require_agent_id() {
  if [ -z "${OC_AGENT_ID:-}" ]; then
    echo "REFUSED: OC_AGENT_ID is not set. Set to: builder|executor|auditor" >&2
    return 1
  fi
  if ! oc_validate_agent_id "$OC_AGENT_ID"; then
    echo "REFUSED: Invalid OC_AGENT_ID='$OC_AGENT_ID'. Must be: builder|executor|auditor" >&2
    return 1
  fi
  return 0
}
...
# Safe one-time migration from legacy shared path to agent-scoped path.
#
# Usage: oc_migrate_legacy_file <legacy_path> <agent_path>
#
# Behavior (Option A — safe migration):
#   - Both exist      → return 1 (conflict, fail closed)
#   - Legacy only     → mv to agent path, emit PATH_MIGRATION, return 0
#   - Agent only      → return 0 (already migrated)
#   - Neither exists  → return 0 (new file, nothing to migrate)
#
oc_migrate_legacy_file() {
  local legacy="$1"
  local target="$2"

  # Already at agent-scoped path — nothing to do
  if [ -e "$target" ] && [ ! -e "$legacy" ]; then
    return 0
  fi

  # Neither exists — new file, nothing to migrate
  if [ ! -e "$target" ] && [ ! -e "$legacy" ]; then
    return 0
  fi

  # Both exist — conflict, fail closed
  if [ -e "$target" ] && [ -e "$legacy" ]; then
    echo "CONFLICT: Both legacy and agent-scoped paths exist:" >&2
    echo "  Legacy: $legacy" >&2
    echo "  Agent:  $target" >&2
    echo "  Resolve manually before proceeding." >&2
    return 1
  fi

  # Legacy exists, agent-scoped doesn't — migrate
  mkdir -p "$(dirname "$target")"
  mv "$legacy" "$target" || {
    echo "ERROR: Failed to migrate $legacy -> $target" >&2
    return 1
  }

  # Emit PATH_MIGRATION event if emit_event is available
  if type emit_event >/dev/null 2>&1; then
    emit_event "PATH_MIGRATION" "from_type=legacy" "to_type=agent_scoped"
  fi
